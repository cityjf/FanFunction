...
namespace FanFunction
{
    ...
    /// 控制函数执行时间,超时返回不继续执行
    /// 调用方法
    /// clsFuncTimeout.EventNeedRun action = delegate(object[] param)
    /// {
    ///     //调用自定义函数
    ///     return Test1(param[0].ToString(), param[1].ToString(), (DateTime)param[2]);
    /// };
    /// clsFuncTimeout ft = new clsFuncTimeout(action, 2000);
    /// var result = ft.doAction("1", "2", DateTime.Now);
    /// </summary>
    public class clsFuncTimeout
    {
        /// <summary> 
        /// 信号量 
        /// </summary> 
        ...
        public clsFuncTimeout(EventNeedRun _action, int _timeout)
        {
